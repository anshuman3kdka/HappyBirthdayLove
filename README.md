...
*   **[v2.0.0]** - "Sonic Depth" Update: Overhauled the procedural global chimes with high-fidelity 2D spatial audio. Replaced the basic stereo panning with a Web Audio `PannerNode` using the HRTF (Head-Related Transfer Function) model. Clicking the screen now maps both X and Y coordinates to the audio listener, allowing you to hear "starlight" appearing above, below, or beside you in a immersive 3D soundscape.

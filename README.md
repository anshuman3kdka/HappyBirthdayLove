...
*   **Same-Address Scene Navigation:** The top tabs are now in-place scene switches, not route links. Clicking Journal, Archive, Projection, or Sky changes the visible room, lets the soundtrack breathe through a two-second fader, and triggers the cosmic background transition while the browser stays on the same URL.
...
*   **Audible Archive Video:** The Archive's central video memory now plays with its own audio at 70% volume, layered over the existing nostalgic background score instead of muting or ducking it. If a browser asks for one more direct interaction before allowing video sound, the player is prepared to retry with sound on the next tap or click while the moment is in view.

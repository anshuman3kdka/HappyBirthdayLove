...
*   **[v1.16.0]** - Format-Independent Architecture: Completely overhauled the media pipeline to dynamically resolve and preload file extensions (`.webp`, `.mp4`, `.avif`, `.webm`, etc.) at runtime across the entire application. The site now relies on format-agnostic asset paths via highly optimized `fetch(HEAD)` and native error-handling fallback checks, ensuring users can simply drop in their own modern media files with the correct base name and everything compiles and plays flawlessly without touching the code.
